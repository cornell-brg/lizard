...
void bthread_init() {
  ...
  if (core_id == 0) {
    for (int i = 0; i < 4; i++) g_thread_flags[i] = 0;
    ...
    while (1) {
      ...
      (*g_thread_spawn_func_ptrs[core_id])(g_thread_spawn_func_args[core_id]);
...
int bthread_spawn(int thread_id, void (*start_routine)(void*), void* arg) {
  ...
  if (thread_id >= ncores || thread_id < 0) return -1;

  // Check to see if the thread is already in use. If so, then return
  // an error.

  if (g_thread_flags[thread_id]) return -1;
...
int bthread_join(int thread_id) {
  ...
  if (thread_id < 1 || thread_id >= 4) return -1;
...
int bthread_join(int thread_id) { return 0; }
