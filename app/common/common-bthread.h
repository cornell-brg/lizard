...
inline int bthread_get_num_cores() {
  int num_cores;
  asm("csrr %0, 0xFC1;" : "=r"(num_cores) :);
  return num_cores;
}

#else

inline int bthread_get_num_cores() { return 1; }
...
inline int bthread_get_core_id() {
  int core_id;
  asm("csrr %0, 0xF14;" : "=r"(core_id) :);
  return core_id;
}

#else

inline int bthread_get_core_id() { return 0; }
