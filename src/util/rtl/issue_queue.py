...
class GenericIssueSlot(Model):
  ...
  def __init__(s, SlotType):
    ...
    """
    s.iface = IssueQueueSlotInterface(SlotType(), SlotType().src0.nbits, SlotType().branch_mask.nbits)
    s.iface.apply(s)
    ...
    @s.combinational
    def match_src():
      s.src0_match_.v = s.curr_.src0_valid and s.notify_call and (not s.curr_.src0_rdy and s.curr_.src0 == s.notify_value)
      s.src1_match_.v = s.curr_.src1_valid and s.notify_call and (not s.curr_.src1_rdy and s.curr_.src1 == s.notify_value)
