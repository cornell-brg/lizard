...
class RV64GSemantics( object ):
  ...
  def execute_sllw( s, rd, rs1, rs2 ):
    s.R[ rd ] = sext(
        Bits(
            32,
            s.R[ rs1 ][:32 ] << ( s.R[ rs2 ][:32 ].uint() & 0x1F ),
            trunc=True ) )
  ...
  def execute_mulh( s, rd, rs1, rs2 ):
    s.R[ rd ] = Bits( XLEN * 2,
                      s.R[ rs1 ].int() * s.R[ rs2 ].int() )[ XLEN:2 * XLEN ]

  @instr
  def execute_mulhsu( s, rd, rs1, rs2 ):
    s.R[ rd ] = Bits( XLEN * 2,
                      s.R[ rs1 ].int() * s.R[ rs2 ].uint() )[ XLEN:2 * XLEN ]

  @instr
  def execute_mulhu( s, rd, rs1, rs2 ):
    s.R[ rd ] = Bits( XLEN * 2,
                      s.R[ rs1 ].uint() * s.R[ rs2 ].uint() )[ XLEN:2 * XLEN ]

  @instr
  def execute_mulw( s, rd, rs1, rs2 ):
    s.R[ rd ] = sext(
        Bits( 32, s.R[ rs1 ][:32 ].int() * s.R[ rs2 ][:32 ].int(),
              trunc=True ) )
  ...
  def execute_div( s, rd, rs1, rs2 ):
    s.R[ rd ] = Bits( XLEN,
                      s.sane_divide( s.R[ rs1 ].int(), s.R[ rs2 ].int(),
                                     XLEN ) )

  @instr
  def execute_divu( s, rd, rs1, rs2 ):
    s.R[ rd ] = Bits(
        XLEN, s.sane_divide( s.R[ rs1 ].uint(), s.R[ rs2 ].uint(), XLEN ) )
  ...
  def execute_divw( s, rd, rs1, rs2 ):
    s.R[ rd ] = sext(
        Bits(
            32,
            s.sane_divide( s.R[ rs1 ][:32 ].int(), s.R[ rs2 ][:32 ].int(),
                           32 ) ) )
  ...
  def execute_divuw( s, rd, rs1, rs2 ):
    s.R[ rd ] = sext(
        Bits(
            32,
            s.sane_divide( s.R[ rs1 ][:32 ].uint(), s.R[ rs2 ][:32 ].uint(),
                           32 ) ) )
  ...
  def execute_rem( s, rd, rs1, rs2 ):
    s.R[ rd ] = Bits( XLEN,
                      s.sane_rem( s.R[ rs1 ].int(), s.R[ rs2 ].int(), XLEN ) )

  @instr
  def execute_remu( s, rd, rs1, rs2 ):
    s.R[ rd ] = Bits( XLEN,
                      s.sane_rem( s.R[ rs1 ].uint(), s.R[ rs2 ].uint(), XLEN ) )

  @instr
  def execute_remw( s, rd, rs1, rs2 ):
    s.R[ rd ] = sext(
        Bits( 32,
              s.sane_rem( s.R[ rs1 ][:32 ].int(), s.R[ rs2 ][:32 ].int(),
                          32 ) ) )
  ...
  def execute_remuw( s, rd, rs1, rs2 ):
    s.R[ rd ] = sext(
        Bits(
            32,
            s.sane_rem( s.R[ rs1 ][:32 ].uint(), s.R[ rs2 ][:32 ].uint(),
                        32 ) ) )
  ...
  def execute_slliw( s, rd, rs1, shamt32 ):
    s.R[ rd ] = sext(
        Bits( 32, s.R[ rs1 ][:32 ] << shamt32.uint(), trunc=True ) )
  ...
  def execute_sraiw( s, rd, rs1, shamt32 ):
    s.R[ rd ] = sext(
        Bits( 32, s.R[ rs1 ][:32 ].int() >> shamt32.uint(), trunc=True ) )
