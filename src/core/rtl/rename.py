...
OutGetRenameRdyCallOutBundle, InGetRenameRdyCallOutBundle = create_PortBundles(
    GetRenameRdyCallOutBundle )
OutRenameRdyCallOutBundle, InRenameRdyCallOutBundle = create_PortBundles(
    RenameRdyCallOutBundle )
...
class RenameTableRTL( Model ):
  ...
  def __init__( s, naregs, npregs ):
    ...
    # RegisterFile
    s.rf = m = RegisterFileRst(
        dtype=Bits( clog2( npregs ) ),
        nregs=naregs,
        const_zero=True,
        reset_values=[ x for x in range( naregs ) ] )
    s.connect_pairs( m.rd_addr[ 0 ], s.get_rename.x, m.rd_data[ 0 ],
                     s.get_rename.p, m.wr_en, s.rename.call, m.wr_addr,
                     s.rename.x, m.wr_data, s.rename.p )
  ...
  def line_trace( s ):
    rename_trace = "rename x{: <{width}} to p{: <{width}}".format(
        s.rename.x, s.rename.p, width=s.nbits ) if s.rename.call else ""
    get_rename_trace = "get_rename x{: <{width}}: p{: <{width}}".format(
        s.get_rename.x, s.get_rename.p,
        width=s.nbits ) if s.get_rename.call else ""
    return "{: <{width}} () {: <{width}}".format(
        rename_trace, get_rename_trace, width=24 )
