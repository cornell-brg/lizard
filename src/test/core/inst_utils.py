...
def gen_br2_src1_dep_test(num_nops, inst, src0, src1, taken):
    return gen_br2_template(8 - num_nops, num_nops, "x1", "x2", inst, src0,
                            src1, taken)
...
def gen_st_value_test(inst, offset, base, data):
    return gen_st_base_data_template(0, 0, "x1", "x2", inst, offset, base,
                                     data)
...
class TestHarness(Model):
    ...
    def __init__(s, ProcModel, src_delay, sink_delay, mem_stall_prob,
                 mem_latency):
...
def run_test(ProcModel,
    ...
    model = TestHarness(ProcModel, src_delay, sink_delay, mem_stall_prob,
                        mem_latency)
    model.elaborate()
