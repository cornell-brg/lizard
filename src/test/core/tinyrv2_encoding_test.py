...
def test_tinyrv2_inst_add():
  check( "add   x1,  x2,  x3", 0b00000000001100010000000010110011,
         "add   x01, x02, x03" )
  check( "add   x01, x02  x03", 0b00000000001100010000000010110011,
         "add   x01, x02, x03" )
...
def test_tinyrv2_inst_addi():
  check( "addi  x1,  x2,  3", 0b00000000001100010000000010010011,
         "addi  x01, x02, 0x003" )
  check( "addi  x1,  x2,  0xff", 0b00001111111100010000000010010011,
         "addi  x01, x02, 0x0ff" )
...
def test_tinyrv2_inst_lui():
  check( "lui   x1,  0xdeadb", 0b11011110101011011011000010110111,
         "lui   x01, 0xdeadb" )


def test_tinyrv2_inst_auipc():
  check( "auipc x1,  0xdeadb", 0b11011110101011011011000010010111,
         "auipc x01, 0xdeadb" )
...
def test_tinyrv2_inst_lw():
  # lw rd, imm(rs1)
  check( "lw    x1,     3(x2)", 0b00000000001100010010000010000011,
         "lw    x01, 0x003(x02)" )
  check( "lw   x13, 0xabc(x4)", 0b10101011110000100010011010000011,
         "lw    x13, 0xabc(x04)" )
  check( "lw   x13,    -3(x4)", 0b11111111110100100010011010000011,
         "lw    x13, 0xffd(x04)" )


def test_tinyrv2_inst_sw():
  # sw rs2, imm(rs1)
  check( "sw    x1,     3(x2)", 0b00000000000100010010000110100011,
         "sw    x01, 0x003(x02)" )
  check( "sw    x1, 0xabc(x2)", 0b10101010000100010010111000100011,
         "sw    x01, 0xabc(x02)" )
...
def test_tinyrv2_inst_jal():
  ...
  check_sym( sym, 0x1000, "jal  x01, label_a",
             0b00000000010000000001000011101111, "jal x01, 0x001004" )


def test_tinyrv2_inst_jalr():
  check( "jalr  x10, x8,  3", 0b00000000001101000000010101100111,
         "jalr  x10, x08, 0x003" )
...
def test_tinyrv2_inst_beq():
  ...
  check_sym( sym, 0x1000, "beq x01, x02, label_a",
             0b01000000001000001000001001100011, "beq x01, x02, 0x0404" )
  check_sym( sym, 0x1000, "beq x01, x02, label_b",
             0b11000000001000001000000001100011, "beq x01, x02, 0x1400" )
