...
from lizard.util.arch import isa
from lizard.util.arch.isa import Isa, FieldSpec, FieldFormat
from lizard.util.arch.assembler import Assembler
from lizard.bitutil import bslice
...
class FenceFormat(FieldFormat):
  ...
  def parse_fence_spec(self, spec):
    assert len(spec) < len(FenceFormat.fence_locs)
    result = Bits(len(FenceFormat.fence_locs), 0)
    for c in spec:
      assert c in FenceFormat.fence_locs
      assert not result[FenceFormat.fence_locs[c]]
      result[FenceFormat.fence_locs[c]] = 1

    return result

  def format_fence_spec(self, value):
    result = ''
    for c in FenceFormat.fence_spec:
      if value[FenceFormat.fence_locs[c]]:
        result += c
    return result

  def parse(self, sym, pc, spec):
    return self.parse_fence_spec(spec)

  def format(self, value):
    return self.format_fence_spec(value)
