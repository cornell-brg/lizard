...
class GlobalBranchPredictor(Model):
  ...
  def __init__(s, interface, hist_idx_nbits, hist_nbits, pht_nbits, hasher='concat'):
    ...
    hist_iface = SynchronousRAMInterface(hist_nbits, 2**hist_idx_nbits, 2, 1, False)
    ...
    s.reg_pc_predict = Register(RegisterInterface(Bits(s.interface.pc_nbits), enable=True))
